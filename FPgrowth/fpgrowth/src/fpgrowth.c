/*----------------------------------------------------------------------
  ...
  History : 2004.11.21 file created from eclat.c
            ...
            2013.10.31 bug in function fpg_adjust fixed (check of mrep)
...
#define VERSION     "version 5.6 (2014.01.08)         " \
...
{                               /* --- search for frequent item sets */
  ...
  if (m <= 0) {                 /* check whether there are items left */
    r = (isr_report(report) < 0) ? -1 : 0; free(rd.set); return r; }
  ...
  if (mode & FPG_FIM16) {       /* if to use a 16-items machine */
    ...
    if (r >= 0) {               /* if a frequent pattern tree */
      ...
      if (r >= 0) r = (isr_report(report) < 0) ? -1 : 0;
...
{                               /* --- main function */
  ...
  k = fpgrowth(tabag, target, algo, mode, (SUPP)supp,
